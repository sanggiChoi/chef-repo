...
class Chef::Handler::Slack < Chef::Handler
  ...
  def report
    ...
    @webhooks['name'].each do |val|
      ...
      Chef::Log.info("sanggi=> Sending report to Slack webhook #{webhook['url']}")
      ...
      Timeout.timeout(@timeout) do
        ...
        #Chef::Log.debug("#{sending_to_slack}")
  ...
  def send_message(message, text_attachment = nil)
    Chef::Log.info("sanggi - Sending send message #{message} to message.local.local #{text_attachment ? 'with' : 'without'} a text attachment")
    uri = URI.parse('http://192.168.13.14:5000/api/1/send_messages')
    http = Net::HTTP.new(uri.host, uri.port)
    req = Net::HTTP::Post.new(uri.path, 'Content-Type' => 'application/json')
    req.body = request_body(message, text_attachment)
    res = http.request(req)
